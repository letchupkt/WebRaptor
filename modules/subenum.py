...
author = "LakshmikanthanK (Letchu) - Enhanced"
version = "3.0"
...
def run(target):
    ...
    try:
        ...
        if enumerator.show_advanced_menu():
            asyncio.run(run_advanced_enumeration(enumerator, target))
        else:
...
def save_advanced_results(enumerator, domain):
    ...
    json_file = f"advanced_subenum_{domain.replace('.', '_')}_{timestamp}.json"
    try:
        json_data = {
            'metadata': {
                'domain': domain,
    ...
    # Save CSV report
    csv_file = f"advanced_subenum_{domain.replace('.', '_')}_{timestamp}.csv"
    try:
        import csv
